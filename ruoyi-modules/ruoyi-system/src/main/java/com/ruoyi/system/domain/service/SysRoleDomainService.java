...
public class SysRoleDomainService {
    ...
    public String getPermsSql(String deptAlias, String userAlias) {
        ...
        if (!scopeCustomIds.isEmpty()) {
            ...
            {
                sqlStr.append(StringUtils.format(" OR {}.dept_id IN ( SELECT dept_id FROM sys_role_dept WHERE role_id = {} ) ", deptAlias, String.join(",", scopeCustomIds)));
            }
            dataScopeSet.add(RolePermEnum.DATA_SCOPE_ALL.getCode());
        }
